...
# ---------------------------------------------------------
DATA_PATH = '../data/SIPaKMed_cropped'
...
# 2. File Loading Helper Functions (Ported from your old code)
# ---------------------------------------------------------
...
# 4. Optimized Attention Extraction (Modern DINOv2 Logic)
# ---------------------------------------------------------
def get_attention_map(model_name, img_tensor):
...
try:
    ...
    try:
        image_source = get_random_local_image(DATA_PATH)
        filename_display = os.path.basename(image_source)
    except Exception as e:
        print(f"Warning: Could not load local image ({e}). Using fallback URL.")
        image_source = "https://upload.wikimedia.org/wikipedia/commons/9/99/Brooks_Chase_Ranger_of_Jolly_Dogs_Jack_Russell.jpg"
    ...
    fig.suptitle(f"File: {filename_display}", fontsize=14, fontweight='bold')
    ...
    axes[3].imshow(map_with_reg, cmap='jet', alpha=0.5, interpolation='bilinear',
                   extent=[0, IMAGE_SIZE[0], IMAGE_SIZE[1], 0])
    axes[3].set_title("Importance Overlay")
    axes[3].axis("off")
